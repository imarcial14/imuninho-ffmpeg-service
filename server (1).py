...
def render():
    ...
    hook_text = data.get('hook', '')[:60].replace("'", " ").replace('"', ' ').replace(':', ' ').replace('\', ' ')
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            text_filter = (
                f"drawtext=fontfile={font}:text='{hook_text}':fontsize=42:fontcolor=white:shadowcolor=black:shadowx=2:shadowy=2:x=(w-text_w)/2:y=h*0.08:enable='between(t\\,0\\,4)'[v1];"
                f"[v1]drawtext=fontfile={font}:text='{cta_text}':fontsize=36:fontcolor=white:shadowcolor=black:shadowx=2:shadowy=2:x=(w-text_w)/2:y=h*0.85:enable='gte(t\\,12)'[vfinal]"
            )
