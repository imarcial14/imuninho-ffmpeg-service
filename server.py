...
Recebe clips via URL, monta o vídeo e faz upload para o Google Drive
...
def download_file(url, dest_path):
    """Baixa arquivo de URL para dest_path."""
        ...
        try:
            ...
            if data.get('music_url'):
                ...
                try:
                    ...
                    logger.warning(f"Trilha falhou, sem música: {e}")
                    music_path = None

            # Monta filtro FFmpeg com overlay de texto
            text_filter = (
                f"drawtext=text='{hook_text}':fontsize=42:fontcolor=white:"
                f"shadowcolor=black:shadowx=2:shadowy=2:"
                f"x=(w-text_w)/2:y=h*0.08:enable='between(t,0,4)'[v1];"
                f"[v1]drawtext=text='{cta_text}':fontsize=36:fontcolor=white:"
                f"shadowcolor=black:shadowx=2:shadowy=2:"
                f"x=(w-text_w)/2:y=h*0.85:enable='gte(t,12)'[vfinal]"
            )

            if music_path and os.path.exists(music_path):
                ffmpeg_cmd = [
                    'ffmpeg', '-y',
                    '-i', clip1_path, '-i', clip2_path, '-i', music_path,
                    '-filter_complex',
            ...
            # Gera token via Service Account
